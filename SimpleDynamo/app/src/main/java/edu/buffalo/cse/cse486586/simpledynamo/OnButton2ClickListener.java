 ...
 *
 */
public class OnButton2ClickListener implements OnClickListener {

    private static final String TAG = OnButton2ClickListener.class.getName();
    ...
    public OnButton2ClickListener(TextView _tv, ContentResolver _cr) {
        mTextView = _tv;
    ...
    private boolean testQuery() {
        try {
            for (int i = 0; i < TEST_CNT; i++) {
                ...
                String[] selectionArgs = {"REMOTE","REMOTE1"};

                Cursor resultCursor = mContentResolver.query(mUri, null, "@", selectionArgs, null);
                ...
                if (resultCursor.moveToFirst()) {
                    do {
                        String key1 = resultCursor.getString(resultCursor.getColumnIndex(KEY_FIELD));
                        String value1 = resultCursor.getString(resultCursor.getColumnIndex(VALUE_FIELD));

                        String text = "<font color=red>" + key1 + "<br/></font>";
                        mTextView.append(Html.fromHtml(text));
                        text = "<font color=blue>" + value1 + "<br/><br/></font>";
                        mTextView.append(Html.fromHtml(text));

                        Log.i(TAG,"looping through cursor: " + key1 + " " + value1);
