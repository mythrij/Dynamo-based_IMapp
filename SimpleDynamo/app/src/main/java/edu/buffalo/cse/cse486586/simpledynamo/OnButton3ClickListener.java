 ...
 *
 */
public class OnButton3ClickListener implements OnClickListener {

    private static final String TAG = OnButton3ClickListener.class.getName();
    ...
    public OnButton3ClickListener(TextView _tv, ContentResolver _cr) {
        mTextView = _tv;
...
//        }
        if (testDelete()) {
            mTextView.append("Delete success\n");
        } else {
            mTextView.append("Delete fail\n");
        }
    }
    ...
    private boolean testDelete() {
        try {
            for (int i = 0; i < TEST_CNT; i++) {
                ...
                int result = mContentResolver.delete(mUri, "*", null);
